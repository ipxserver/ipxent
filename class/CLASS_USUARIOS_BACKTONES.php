...
class CLASS_USUARIOS_BACKTONES
{
	...
	FN_NET_LOGGER("QUERY:$vl_sql_renovacion");
        ...
        if ($vl_resultado == true)
        {
			$vf_precio= FN_SEND_ASYNC($this->vlc_msisdn,$vp_id_tono, 10, 1) + 0;
	...
	function MTD_TRY_AND_BUY($vp_msisdn, $vp_tono, $vp_nombre_tono)
	{
		global $vg_db_conexion;
		$vl_short = 0;
		
		$vl_sql = "SELECT number FROM short_numbers WHERE number NOT IN (SELECT short FROM regalos_pendientes WHERE msisdn='".$vp_msisdn."') LIMIT 1";
		$vf_arreglo_short = FN_RUN_QUERY($vl_sql, 1,$vg_db_conexion);
		$vl_short =$vf_arreglo_short[0][0];
		FN_NET_LOGGER("Msisdn:$vp_msisdn.Short1: $vl_short ");
		if($vl_short == 0)
		{
			$sql = "SELECT short FROM regalos_pendientes WHERE msisdn='$vp_msisdn' ORDER BY fecha LIMIT 1";
			$vf_arreglo_short = FN_RUN_QUERY($sql, 1,$vg_db_conexion);
			$vl_short =$vf_arreglo_short[0][0];
		}
		 FN_NET_LOGGER("Msisdn:$vp_msisdn.Short2: $vl_short ");
		if($vl_short > 0)
		{
			$query = "INSERT INTO regalos_pendientes ( msisdn, fecha, tono, short, numero, tipo, tono_promocion, clave, confirmado) VALUES ".
                "('".$vp_msisdn."', unix_timestamp(), $vp_tono, $vl_short, '' , 32,NULL,'', 1)";
			$vl_resultado = false;
			$vl_resultado = FN_RUN_NONQUERY($query,$vg_db_conexion);
			
			$query2 = "INSERT INTO historial_sms (msisdn,historial,fecha, clave, push) VALUES ".
				"('".$vp_msisdn."', 'gift=1', unix_timestamp(), '', 0)";
			
			$vl_resultado = false;
			$vl_resultado = FN_RUN_NONQUERY($query2,$vg_db_conexion);
				
			$vl_mensaje_sms  = "Desea realizar un Try and Buy del tono '".$vp_nombre_tono."'?\n1-Aceptar\n2-Cancelar.";
			FN_NET_SMS_2($vl_mensaje_sms,$vp_msisdn, $vl_short);
		}

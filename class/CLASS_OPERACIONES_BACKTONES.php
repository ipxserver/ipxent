    ...
    function MTD_COMPRAR_BACKTONES()
    {
        ...
        if (trim($vf_resultado_compra) == "0")
        {
			...
			##	 $vf_dias_renovacion = $obj_usuarios_backtones->MTD_DB_GET_DIAS_RENOVACION($this->vlc_id_tono);
        	    ...
        	    $vf_precio= FN_SEND_ASYNC($this->vlc_msisdn,$this->vlc_id_tono, 10, 1) + 0;
		        $vf_precio=$vf_precio/100;
            	$precio = number_format($vf_precio, 2, '.', '');

                 $arreglo_tono = $this->MTD_DB_LISTAR_TONO();
                 $vf_mensaje_sms="Compraste el tono ".$arreglo_tono[0][1]." - ".$arreglo_tono[0][2]." que estara activo por ".$vf_dias_renovacion." dias.Costo mensual de bs $precio.";

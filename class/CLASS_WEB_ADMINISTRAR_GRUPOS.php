        ...
        */
        $vlf_sql="
            SELECT
                ...
                a.nombre,
                (CASE WHEN ng.groupid > 0 THEN 1 else 0 end) as asignado
                FROM asignados as a, numeros_grupos as ng
                WHERE
		a.ani = ng.ani AND ng.groupid = $vp_id_grupo AND
		...
		from asignados 
		where ani not in 
		    (select ani from numeros_grupos where groupid in 
			(select id from grupos where msisdn='".$this->vlc_msisdn."')
		    )
            UNION

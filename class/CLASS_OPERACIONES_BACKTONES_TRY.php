...
class CLASS_OPERACIONES_BACKTONES
{
    ...
    function MTD_COMPRAR_BACKTONES()
    {
        ...
        if (trim($vf_resultado_compra) == "0")
        {
			...
			##	 $vf_dias_renovacion = $obj_usuarios_backtones->MTD_DB_GET_DIAS_RENOVACION($this->vlc_id_tono);
                 ...
                 $vf_mensaje_sms="Compraste el tono ".$arreglo_tono[0][1]." - ".$arreglo_tono[0][2]." que estara activo por ".$vf_dias_renovacion." dias.Costo mensual de bs 3,5.";
				 ...
				 $vf_mensaje_sms1= "El tono que compraste esta esta sujeto a la reproduccion de un pre-backtone,para desactivar el pre-backtone llama al *611";
				 FN_NET_SMS($vf_mensaje_sms1, $this->vlc_msisdn);
 ...
 bearer-required=\"true\"
    ...
    function MTD_DESCARGAR_BACKTONES_OLD()
    {
	    ...
	    $url = "http://backtones.com.bo/bajar.php?msisdn=".$this->vlc_msisdn."&amp;id=".$codigo;
            ...
            <!DOCTYPE pap PUBLIC \"-//WAPFORUM//DTD PAP//EN\"
              ...
              <push-message push-id=\"9fjeo39jf084@pi.com\"
              progress-notes-requested=\"false\">
                <address address-value=\"WAPPUSH=+".$this->vlc_msisdn."/TYPE=PLMN@entel.com.py\">
                </address>
                <quality-of-service
                priority=\"low\"
                delivery-method=\"unconfirmed\"
    ...
    function MTD_RETORNOS_RESPUESTA_ASYNC($vp_respuestas)
    {
        ...
        {
                case "1060" :
					$vl_sql2 = "SELECT estado FROM sistema WHERE feature='TRYANDBUY' AND medio='WEB'";
					$vl_arreglo= array();
					$vl_arreglo= FN_RUN_QUERY($vl_sql2, 1, $vg_db_conexion);
					if($vl_arreglo)
					{
						$vl_estado = $vl_arreglo[0][0];
					}
					
					FN_NET_LOGGER("Msisdn:".$this->vlc_msisdn.".Tono:".$this->vlc_id_tono.".Estado TryAndBuy: ".$vl_estado." estado",0);
					...
					if($vl_estado)
					{
						...
						$vl_sql = "SELECT cantidad as cantidadBuy FROM blacklist_tryandbuy WHERE msisdn = '".$this->vlc_msisdn."' AND fecha > unix_timestamp() - $diasTry*24*3600";
						$vl_arreglo= array();
						...
						FN_NET_LOGGER("Msisdn:".$this->vlc_msisdn.".Tono:".$this->vlc_id_tono.".Try And BUY.Cantidad:$vl_cantidad", 0);
						$obj_usuarios_backtones = new CLASS_USUARIOS_BACKTONES();
						if( $vl_cantidad < $cantidadTry )
						{

							FN_NET_LOGGER("Msisdn:".$this->vlc_msisdn."try");
							$obj_usuarios_backtones->MTD_TRY_AND_BUY($this->vlc_msisdn, $this->vlc_id_tono, $arreglo_tono[0][2]);
						}
					}
    ...
    function MTD_APLICAR_TEMPLATE_POPUP_COMPRA()
    {
        ...
        $vl_titulo_facebook="Ha comprado el Ringback Tone Entel  $autor - $tono  ";        
...
*/
	...
	t.id=".$this->vlc_id_tono." $vg_conf_filtro_web ;";
        ...
        $vlf_titulo_head="Ringback Tones Entel | $vlf_nombre_artista - $vlf_nombre_tono ";
